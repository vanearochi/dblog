...
class Blog(db.Model):
    ...
    comment = db.ListProperty(db.Key, default=None)
...
class Handler(RequestHandler):
    ...
    def is_logged_user_the_author(self, blog_id, veryfing_action=False):
        cookie_value = self.request.cookies.get("name")
...
class EditSinglePost(Handler):
    ...
    def get(self, blog_id):
        ...
        template_name = "blog_entry.html"
        if self.valid_login_session() is True:
            self.is_logged_user_the_author(blog_id, veryfing_action=True)
...
class LikePost(Handler):
    ...
    def post(self, blog_id):
        ...
        if self.valid_login_session() is True:
            ...
            #         like_entity.put()
            #         print post_entity.like
            #         like_len = len(post_entity.like)
            #         for like in post_entity.like:
            #             print like
            #             if like == user:
            #                 print "borra"
            #                 post_entity.like.remove(like)
    ...
    def post(self, blog_id):
        ...
        if self.valid_login_session() is True:
            ...
            if comment_status is True:
                ...
                comment_entity.put()
                post_entity.comment.append(user_key)
                post_entity.put()
                print post_entity.comment
                self.redirect_to("NewCreatedPost", blog_id=blog_id)
